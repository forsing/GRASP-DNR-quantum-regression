...
# DNR (Deep Network Representation)

# GRASP + kvantna regresija, deterministički, strukturno
# Varijanta: DNR – Deep Network Representation (višeslojna propagacija, ponderisana suma)
 
"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
DNR (Deep Network Representation): Z = Σ_t α^t · (A_norm^t @ X), X = Laplacian prvih k;
agregacija više „slojeva“ susedstva u jedan embedding.
"""
...
DNR_ALPHA = 0.3 # 0.3 alpha for DNR
DNR_LAYERS = 5  # 5 layers for DNR
...
def compute_dnr_embeddings(M, k=EMBED_DIM):
...
def main():
    ...
    emb = compute_dnr_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (DNR + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (DNR + kvantna regresija, deterministički, strukturno):
    (8, 11, 22, 23, 26, 34, 37)

    Score: 0.4992733051134129
    """
...
X: prvih k sopstvenih vektora Laplaciana (39×k).
A_norm: simetrično normalizovana A+I (39×39).
Višeslojna agregacija: Z = X + α·(A_norm@X) + α²·(A_norm²@X) + … (DNR_ALPHA=0.3, DNR_LAYERS=5).
